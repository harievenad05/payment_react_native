...
import {
  getLoginEndpoint, getRegisterEndpoint,
...
export function* CustomerRegisterWatcher() {
  yield takeLatest(Action.LoginAction.SIGNUP, registerRequest);
}
...
export function* registerRequest(action) {
  try {
    let requestData = action.payload.details;
    // debugger;
    console.log('debug >>>>>> PostData', requestData);
    const result = yield call(() =>
      request(getRegisterEndpoint(), HTTP_METHODS.POST, requestData),
    );
    console.log('debug >>>>>> Responsssseeeeee', result);
